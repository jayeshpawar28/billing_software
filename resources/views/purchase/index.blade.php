                    ...
                    <div class="bg-light rounded h-100 p-4">
                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @php
                                        $total_item = count($temp_pur_data);
                                        $total_qty = 0;
                                        $total_amount = 0;
                                    @endphp
                                    @foreach ($temp_pur_data as $data)
                                        <tr>
                                            <th scope="row">{{ $loop->iteration }}</th>
                                            <td>{{ $data->product->product_name }}</td>
                                            <td>
                                                <form method="POST"
                                                    action="{{ route('temp_purchase_update', $data->temp_purchase_pk) }}">
                                                    @csrf
                                            ...
                                            <td><a
                                                    href="{{ route('temp_purchase_delete', $data->temp_purchase_pk) }}">Delete</a>
                                            </td>
                                        </tr>
                                        @php
                                            $total_qty += $data->qty;
                            ...
                            <div class="text-center m-3">
                                <a class="btn btn-success" href="{{ route('confirm_pruchase') }}">Create Bill</a>
                            </div>
