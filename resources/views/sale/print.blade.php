...
<body>
    <header class="clearfix">
        ...
        <div id="project">
            <h4>Bill To :</h4>
            <div><span>CUSTOMER </span> {{$data->customer->customer_name}}</div>
            <div><span>CONTACT</span> {{$data->customer->mobile}}</div>	
            <div><span>ADDRESS</span> {{$data->customer->address}}</div>
            <div><span>DATE</span> {{ \Carbon\Carbon::parse($data->sale_date)->format('d-m-Y')}}</div>
    ...
    <main>
        ...
        <div id="payment_section">
            ...
            <div id="notices">
                <div>NOTICE:</div>
                <div class="notice">Please make a pending payment within 30 days from the invoice date.</div>
            </div>
        </div>
    ...
    <footer>
        Invoice was created on a Computer.
    </footer>
</body>
</html>
